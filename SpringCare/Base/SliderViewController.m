...
- (id)init{
    if (self = [super init]){
        _LeftSContentOffset = ScreenWidth - 60;
        _RightSContentOffset = 160;
        _LeftSContentScale = 0.8;
        _RightSContentScale = 0.8;
...
- (void)viewDidLoad
{
    ...
    [self showContentControllerWithModel:@"RootViewController"];
...
- (CGAffineTransform)transformWithDirection:(RMoveDirection)direction
{
    ...
    CGAffineTransform transT = CGAffineTransformMakeTranslation(translateX, 0);//设置偏移
    CGAffineTransform scaleT = CGAffineTransformMakeScale(transcale, transcale);//设置缩放，分别为x方向，y方向
