...
NSData* EncodePCMToAMR(char* data, int maxLen,int nChannels, int nBitsPerSample ,NSString * voiceName)
{
    char* oldBuf = data;
    /* input speech vector */
	short speech[160];
    ...
    NSString * wavFile= [chat_VoiceCache_path stringByAppendingPathComponent: voiceName];
...
NSData* EncodeWAVEToAMR(NSData* data, int nChannels, int nBitsPerSample,NSString* voiceName)
{
    if (data==nil){
        return nil;
    }
    ...
    return EncodePCMToAMR(buf,maxLen- nPos,nChannels,nBitsPerSample,voiceName);
}
